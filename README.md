...
audience: 비개발자 광고 실무자 (기획/AE · 크리에이티브 · 미디어 · 퍼포먼스 · 커머스 · 콘텐츠)
courses: 8
electives: 1
brand_colors:
  navy: "#1E293B"
...
> 코딩 지식 없이 광고 실무(리서치·크리에이티브·미디어·콘텐츠·퍼포먼스·커머스·자동화)를 AI 에이전트 팀으로 처리하는 **8개 핵심 과정 + 선택 학습** 교육 프로그램.
...
| C7 | **퍼포먼스 마케팅 하네스** — 채널별 전문가를 골라 부르고, 성과를 한 장의 리포트로 | 중급 | 전문가 풀 | [교재](course7-performance/교재/C7_교재.md) | [퍼포먼스 월간리포트](course7-performance/사례/C7_퍼포먼스_월간리포트.md) |
| C8 | **브랜드 스케일업·커머스 하네스** — 광고를 넘어, 제품 기획부터 유통·런칭까지 | 심화 | 계층적 위임 | [교재](course8-scaleup/교재/C8_교재.md) | [스케일업 런칭플랜](course8-scaleup/사례/C8_스케일업_런칭플랜.md) |

> C2에서 업무 자동화로 빠른 실전 이득을 본 뒤, C3~C6으로 캠페인 실무(조사·크리에이티브·미디어·콘텐츠)를, C7~C8로 **성과 운영과 브랜드 성장**(퍼포먼스·커머스 스케일업)까지 확장합니다.
> 가상 브랜드 **"제로톡"**(제로 슈거 스파클링 음료)이 전 과정을 관통해 학습 몰입도를 높입니다.

## 선택 학습 (Elective)

영상·시나리오(대본) 제작이 실제 업무인 실무자를 위한 선택 과정입니다. 핵심 과정을 마친 뒤 필요할 때 수강하세요.

| # | 과정 | 난이도 | 하네스 패턴 | 교재 | 완성형 사례 |
|---|------|:---:|------|------|------|
| E1 | **Seedance 영상 콘텐츠 하네스** — 대본에서 숏폼 영상까지 | 선택·심화 | 파이프라인(영상) | [교재](elective1-video/교재/E1_교재.md) | [Seedance 영상 캠페인](elective1-video/사례/E1_시드댄스_영상캠페인.md) |
...
영상·시나리오 제작이 필요한 실무자는 트랙 완료 후 **E1**을 추가 수강합니다.
...
- **공유 에이전트 풀** (`.claude/agents/`, 12종): 리서처 · 인사이트 에디터 · 카피라이터 · 아트디렉터 · 비주얼 디자이너 · 미디어 플래너 · 콘텐츠 크리에이터 · **퍼포먼스 마케터**(performance-marketer) · **커머스 전략가**(commerce-strategist) · 문서 프로듀서 · QA 리뷰어 · 영상 감독(video-director, 선택 학습용)
- **방법 스킬** (`.claude/skills/`, 12종): 각 팀원의 "업무 매뉴얼" (`performance-marketing` · `commerce-strategy` 포함, Seedance 영상 매뉴얼 `seedance-video`는 선택 학습용)
- **오케스트레이터 스킬** (9종): 과정별 "팀장" — `starter-harness` · `automation-harness` · `research-harness` · `creative-harness` · `media-harness` · `content-harness` · `performance-harness` · `scaleup-harness` · `video-content-harness`(선택)
...
- 사이트 빌드: `cd _site_build && node build_site.js` → `docs/` 에 정적 사이트 생성 (GitHub Pages 대응)
...
- 패턴 색상 코드(프론트매터)로 과정별 테마 색을 입히면 협업 패턴이 시각적으로 구분됩니다.
- 선택 학습(E1)은 랜딩의 별도 "선택 학습" 섹션과 사이드바 하단에 노출됩니다.
